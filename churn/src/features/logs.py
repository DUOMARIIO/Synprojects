...
class LogsPrep:
    ...
    def logs_prepare(self) -> pd.DataFrame:    
        ...
        if self.n_semester == 2: # Будем сравнивать с самим собой
            logs_except_last = logs_last.copy() 
        else:
        ...
        #Заполняем нулями для последующего сокращения размерности
