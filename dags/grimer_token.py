...
'''
Обновление токена для автоматизации  отчета по Лубянскому гримеру
'''
...
with DAG(dag_id='grimer_token',
        tags = ['grimer','docker'],
        description = 'Обновление токена для автоматизации  отчета по Лубянскому гримеру' ,
    ...
    task = DockerOperator(
            task_id='get_token',             # Название task
            image='grimer_img:5',            # Название и тег запускаемого образа
            command='python /grimer_auto/api_app/main_token.py', # Запускаемая команда внутри контейнера
